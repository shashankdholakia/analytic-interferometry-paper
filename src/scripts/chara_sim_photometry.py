...
plt.imshow(fim_vis[0:cutoff_n,0:cutoff_n],vmin=-1e7,vmax=1e7,  cmap='RdBu_r', )
plt.colorbar()
...
fig = plt.figure()
plt.imshow(fim_cp[0:cutoff_n,0:cutoff_n],vmin=-1e9,vmax=1e9,  cmap='RdBu_r', )
plt.colorbar()
tick_labels = jnp.arange(1,CUTOFF+1)
tick_positions = [nmax(i) for i in tick_labels-1]
plt.xticks(tick_positions, tick_labels)
...
fig = plt.figure()
plt.imshow((fim_cp+fim_vis)[0:cutoff_n,0:cutoff_n],vmin=-jnp.max(fim_cp+fim_vis),vmax=jnp.max(fim_cp+fim_vis),  cmap='RdBu_r', )
...
fig = plt.figure()
plt.imshow(fim_lc[0:cutoff_n,0:cutoff_n],vmin=-1e9,vmax=1e9,  cmap='RdBu_r', )
...
fig = plt.figure()
plt.imshow(total_fim[0:cutoff_n,0:cutoff_n],vmin=-1e9,vmax=1e9,  cmap='RdBu_r', )
