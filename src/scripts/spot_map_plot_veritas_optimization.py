...
def tv_l1_penalty(pix, lam_tv=6e-4, lmax=15):
...
plt.savefig(paths.figures / 'veritas_optimization.pdf', bbox_inches='tight', dpi=300)

plt.clf()
fig = plt.figure()
for losses, snr in zip(losses_all, SNR):
    plt.plot(losses)
    plt.xlabel("Iteration")
    plt.ylabel("Loss")
    plt.title(f"Loss Curves (SNR={snr})")
    plt.savefig(paths.figures / f"loss_curves_snr_{snr}.pdf", bbox_inches='tight', dpi=300)