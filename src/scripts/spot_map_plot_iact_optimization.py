...
def tv_l1_penalty(pix, lam_tv=7e-8, lmax=15):
...
# VERITAS Simulation
#################################
print("Setting up VERITAS Simulation")
...
latitude = 34.2249
veritas = earth + wgs84.latlon(latitude * N, 118.0564 * W, elevation_m=1740)
...
for i, snr in enumerate(SNR):
    ...
    print("Evaluating loss function once to compile for VERITAS")
    ...
    print("Starting optimization for VERITAS")
    with tqdm(range(2000)) as pbar:
        for i in pbar:
            ...
            losses.append(loss)
    print("Optimization completed for VERITAS")
    ...
    ax.set_title(f"VERITAS SNR={snr}", fontsize=14)
...
latitude = 28.7134
#fudge the longitude since it's not observable at this time
#from la palma
...
for i, snr in enumerate(SNR):
    ...
    ax.tick_params(axis='x', labelbottom=False) # Hide x-axis tick labels
    ax.tick_params(axis='y', labelleft=False)   # Hide y-axis tick labels
    ax.set_title(f"CTA North SNR={snr}", fontsize=14)
    #plt.colorbar(pcm, ax=ax, orientation='horizontal', fraction=0.046, pad=0.04)
    losses_all.append(losses)
    
plt.savefig(paths.figures / 'iact_optimization.pdf', bbox_inches='tight', dpi=300)
